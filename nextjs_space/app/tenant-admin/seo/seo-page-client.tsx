...
import { Package, FileText, Home, Search, CheckCircle, AlertCircle } from 'lucide-react';
...
export function SeoPageClient({ tenantId, baseUrl, products, posts, pageSeo }: SeoPageClientProps) {
    const [selectedProduct, setSelectedProduct] = useState<ProductItem | null>(null);
    const [selectedPost, setSelectedPost] = useState<PostItem | null>(null);
    const [selectedPage, setSelectedPage] = useState<{ key: string; name: string; path: string } | null>(null);
    const [localPageSeo, setLocalPageSeo] = useState<Record<string, SeoData>>(pageSeo || {});
    ...
    const handleSaveProductSeo = async (seo: SeoData) => {
        ...
        const res = await fetch(`/api/tenant-admin/seo/products/${selectedProduct.id}`, {
            method: 'PUT',
                            ...
                            <div className="divide-y">
                                {STATIC_PAGES.map((page) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-2 flex-shrink-0">
                                            <SeoStatusBadge hasCustomSeo={hasSeo(localPageSeo[page.key])} />
                                            <Button
                                                variant="outline"
                                                size="sm"
