...
`)), s.push(l);
  ...
  } function $(e) { let t = [], s = e; for (; s.length > 0;) { let a = s.length, i = "", o = 0, n = "", l = "", d = s.indexOf("**"); if (d >= 0 && d < a) { let r = s.indexOf("**", d + 2); r >= 0 && (a = d, i = "bold", o = r + 2 - d, n = s.slice(d + 2, r)); } let h = s.indexOf("`"); if (h >= 0 && h < a) { let r = s.indexOf("`", h + 1); r >= 0 && (a = h, i = "code", o = r + 1 - h, n = s.slice(h + 1, r)); } let p = s.indexOf("["); if (p >= 0 && p < a) { let r = s.indexOf("](", p); if (r >= 0) { let u = s.indexOf(")", r + 2); u >= 0 && (a = p, i = "link", o = u + 1 - p, n = s.slice(p + 1, r), l = s.slice(r + 2, u)); } } if (i !== "bold") { let r = s.indexOf("*"); if (r >= 0 && r < a && s[r + 1] !== "*") { let u = s.indexOf("*", r + 1); u >= 0 && s[u - 1] !== "*" && (a = r, i = "italic", o = u + 1 - r, n = s.slice(r + 1, u)); } } if (a > 0) C(t, s.slice(0, a)); if (!i) { break } switch (i) { case "bold": { let r = document.createElement("strong"); r.textContent = n, t.push(r); break } case "italic": { let r = document.createElement("em"); r.textContent = n, t.push(r); break } case "code": { let r = document.createElement("code"); r.textContent = n, t.push(r); break } case "link": { let r = document.createElement("a"); r.textContent = n, r.href = K(l), r.target = "_blank", r.rel = "noopener noreferrer", t.push(r); break } }s = s.slice(a + o); } return t } function C(e, t) {
    let s = t.split(`
