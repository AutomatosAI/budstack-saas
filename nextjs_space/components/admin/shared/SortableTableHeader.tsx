 ...
 */
function getNextSortLabel(currentColumn: string | null, currentOrder: SortOrder, targetColumn: string): string {
...
>(({
  ...
  const SortIcon = () => {
    ...
    if (currentOrder === 'asc') {
      return (
        <ArrowUp
          className={cn(iconClasses, 'animate-in fade-in zoom-in-50 duration-150')}
          aria-hidden="true"
        />
      );
    }
  ...
  return (
    ...
    >
      <button
        ...
        aria-sort={isActive ? (currentOrder === 'asc' ? 'ascending' : currentOrder === 'desc' ? 'descending' : 'none') : undefined}
        className={cn(
