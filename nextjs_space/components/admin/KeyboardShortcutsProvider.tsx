...
import { Command, Search, Home, Users, Package, ShoppingCart, Slash, Zap } from 'lucide-react';
...
export function KeyboardShortcutsProvider({ children, theme }: KeyboardShortcutsProviderProps) {
  ...
  const focusSearch = useCallback(() => {
    const searchInput = document.querySelector<HTMLInputElement>('input[type="text"][placeholder*="Search"], input[type="search"]');
    if (searchInput) {
      searchInput.focus();
      searchInput.select();
    }
      ...
      if (!isInput) {
        ...
        for (const shortcut of shortcuts) {
          if (shortcut.keys.length === 2 &&
              ...
              shortcut.keys[1].toUpperCase() === newSequence[newSequence.length - 1]) {
            e.preventDefault();
            shortcut.handler();
            setSequenceKeys([]);
      ...
      >
            ...
            return (
              <div key={category} className="space-y-3">
                ...
                <div className={cn(
                  ...
                  <CategoryIcon className="w-4 h-4 text-white/80" aria-hidden="true" />
                          ...
                          <React.Fragment key={idx}>
                            ...
                            {idx < shortcut.keys.length - 1 && (
                              <span className="text-white/40 text-xs font-mono mx-0.5">
                                {shortcut.keys.length === 2 && idx === 0 && key !== '⌘' && key !== 'Ctrl' ? 'then' : '+'}
                              </span>
                            )}
                          </React.Fragment>
                        ))}
        ...
        <div className={cn(
          ...
          <p className="text-xs text-white/50 font-mono text-center">
            {isMac ? '⌘ Command' : 'Ctrl'} key • Platform: {isMac ? 'macOS' : 'Windows/Linux'}
          </p>
        </div>
      </DialogContent>
    </Dialog>
...
export function KeyboardHint({ keys, theme = 'tenant-admin', className }: KeyboardHintProps) {
  const themeStyles = {
    'super-admin': {
      bg: 'bg-slate-700/50',
