...
- [ ] US-005 — Wrap + tenant-scope `app/api/customer/profile/route.ts` GET+PATCH in `withAuth` (`:25`/`:79` no-tenant `findFirst` closed by the bound host tenant) + unit test; cross-tenant real-DB proof deferred to US-011/US-012 (AC-3)
- [ ] US-006 — Migrate super-admin routes (~25 of 33) → `withSuperAdmin`/`withSuperAdminParams`; delete hand-rolled blocks; read `user`/`tenantId` from context (AC-1/AC-1a)
