...
| 0.3 | 2026-07-10 | Claude (with Gerard) | **Part B shipped**: outcome persisted on `consultation_questionnaires` (`idDocumentStatus`/`idDocumentError`/`idDocumentUpdatedAt`, idempotent migration) via shared non-throwing `lib/verification/id-document-status.ts`, written from BOTH upload sites (registration inline + `verify/id-document` re-upload route, which previously had NO UI caller); customer dashboard failure banner + `ReUploadIdDocument` CTA (AC-B3); tenant-admin customers list badge (AC-B2; detail page has no verification section yet — follow-up); unit tests (AC-B4). |
| 0.4 | 2026-07-10 | Claude (with Gerard) | **Part C shipped**: recursive write-contract sanitizer `lib/templates/strip-signed-urls.ts` (AC-C1) replaces the fragile per-shape `stripSignedUrl` loops in both `app/api/tenant-admin/branding/route.ts` and `app/api/super-admin/templates/[id]/branding/route.ts`, walking the full persisted layout/design-system/pageContent/settings so a signed URL can't hide in a config shape the old key lists didn't anticipate; `getFileUrl` caller audit (AC-C2) found 3 statically-rendered pages (`app/marketplace/page.tsx`, `app/learn/page.tsx`, `app/learn/[slug]/page.tsx`) baking signed URLs into HTML cached indefinitely — added `revalidate = 1800` (under the 3600s signature life) to each; unit tests (AC-C3). |
