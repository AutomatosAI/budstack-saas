...
| 0.5 | 2026-07-10 | Claude (with Gerard) | **Part A incident + fix**: the sidecar CRASH-LOOPED in prod (`exited with code 1 — restarting in 5s`) — `scripts/email-worker.ts` imported `../lib/encryption`, a module long moved to `lib/security/encryption` (never noticed because nothing had EVER run the worker; scripts/ sits outside the build's type-check graph). Fixed that import + 5 more stale pre-reorg paths across `audit-drgreen-keys` / `seed-s3-templates` / `audit-drgreen-client-ids`, and added `tests/unit/scripts-import-resolution.test.ts` so any scripts/ relative import that stops resolving fails CI. |
