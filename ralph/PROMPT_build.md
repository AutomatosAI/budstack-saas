...
- **Plans do not exist yet.** US-011 creates `tenants.plan` + `resolveTenantPlan` + `requirePlanFeature("seoPro")`. Every C story is server-gated on it; UI locks are presentation, the gate is the boundary. Fail closed: unreadable plan = basic.
